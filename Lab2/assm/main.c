...
void add_cmd(const char *cmd, int operand)
{
    ...
    else
    {
        if (asm_cnt < 10)
    		fprintf(output, "0");
    	fprintf(output, "%d %s ", asm_cnt, cmd);
    	if (operand < 10)
    		fprintf(output, "0");
    	fprintf(output, "%d\n", operand);
    }
...
void fix_goto()
{
    rename("tmp.txt", "tmp1.txt");
    output = fopen("tmp.txt", "w"); 
    FILE *filer = fopen("tmp1.txt", "r");
    char mass[50] = "\0";
    while (1) {
		fscanf(filer, "%s", mass);
        if (feof(filer))
...
void translating(const char *filename)
{
    ...
    count_cnt = (int *) malloc(sizeof(*count_cnt) * (cnt + 1));
    ...
    for (int i = strlen(vars) - 1; i >= 0; i--) {
        ...
        tmp = var_value[i] == 0 ? 1 : var_value[i];
        while(tmp < 1000)
        {
        	tmp *= 10;
        	fprintf(output, "0", var_value[i]);
		}
        fprintf(output, "%d\n", var_value[i]);
    }
